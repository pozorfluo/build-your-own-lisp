 ...
 */
struct hmap *hmap_new(const unsigned int n, const ValueDestructor destructor)
{
	//---------------------------------------------- advertised capacity
	...
	struct hmap_init hashmap_init = {
	    {0}, {NULL}, destructor, n, PROBE_LIMIT, capacity, 0, stats_init};
...
int main(void)
{
	...
	struct hmap *hashmap = hmap_new(n, free); // xfree);
	//----------------------------------------------------------- mock entry
