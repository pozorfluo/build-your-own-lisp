...
static int mpc_parse_run(mpc_input_t *i,
                         mpc_parser_t *p,
                         mpc_result_t *r,
                         mpc_err_t **e,
                         int depth)
{
	...
	case MPC_TYPE_MANY:
		...
		while (mpc_parse_run(i, p->data.repeat.x, &results[j], e, depth + 1)) {
			...
			if (j == MPC_PARSE_STACK_MIN) {
				...
				results_slots = j + j / 2;
				results       = mpc_realloc(
				    i, results, sizeof(mpc_result_t) * results_slots);
			}
		}
	...
	case MPC_TYPE_AND:
		...
		for (j = 0; j < p->data.and.n; j++) {
			if (!mpc_parse_run(
			        i, p->data.and.xs[j], &results[j], e, depth + 1)) {
				mpc_input_rewind(i);
				for (k = 0; k < j; k++) {
...
static mpc_val_t *mpcf_re_escape(mpc_val_t *x, void *data)
{
	...
	if (s[0] == '^') {
		...
		if (mode & MPC_RE_MULTILINE) {
			...
			return mpc_and(
			    2, mpcf_snd, mpc_soi(), mpc_lift(mpcf_ctor_str), free);
		}
	}
...
mpc_parser_t *mpc_re_mode(const char *re, int mode)
{
	...
	mpc_define(Regex,
	           mpc_and(2,
	                   ...
	                   Term,
	                   mpc_maybe(mpc_and(
	                       2, mpcf_snd_free, mpc_char('|'), Regex, free)),
	...
	mpc_define(Range,
	           mpc_apply(mpc_many(mpcf_strfold,
...
static const char *mpc_escape_output_c[] = {"\\a",
                                            "\\b",
                                            "\\f",
                                            "\\n",
                                            "\\r",
...
{
	mpc_val_t *y = mpcf_escape_new(
	    x, mpc_escape_input_raw_cstr, mpc_escape_output_raw_cstr);
	free(x);
	return y;
}
	...
	if (is_number(x)) {
		...
		while (st->parsers_num <= i) {
			...
			if (st->parsers[st->parsers_num - 1] == NULL) {
				return mpc_failf(
				    "No Parser in position %i! Only supplied %i Parsers!",
				    i,
				    st->parsers_num);
			}
		}
...
mpc_parser_t *mpca_grammar_st(const char *grammar, mpca_grammar_st_t *st)
{
	...
	mpc_define(Grammar,
	           mpc_and(2,
	                   ...
	                   Term,
	                   mpc_maybe(mpc_and(
	                       2, mpcf_snd_free, mpc_sym("|"), Grammar, free)),
	                   mpc_soft_delete));
